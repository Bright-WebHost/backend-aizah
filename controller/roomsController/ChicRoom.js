...
const ChicSchema=require('../../model/roomsSchema/ChicSchema');



const ChicInsert=async(req,res)=>{
    ...
    try{
          ...
          const data=await new ChicSchema({checkin,checkout,adults,Children,fname,lname,email,phone,city,code,guests,night,totalprice,paymentID,children,price,roomname,postbook});
...
const ChicView=async (req,res) => {
    try{
        const data=await ChicSchema.find();
...
const ChicUpdate = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updatedData = await ChicSchema.findByIdAndUpdate(
            id,
            { 
                $set: { 
...
const ChicDelete = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedData = await ChicSchema.findByIdAndDelete(id);
...
module.exports={ChicInsert,ChicView,ChicUpdate,ChicDelete};